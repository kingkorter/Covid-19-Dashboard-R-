...
df <- df %>% rename(HYPERTENSION= HIPERTENSION)
    ...
    sidebarLayout(
        ...
        mainPanel(
            tabsetPanel(
                ...
                tabPanel("Fatality Factors", plotOutput("graphs4"))
            )
        )
    )
...
server <- function(input, output) {
  ...
  output$graphs4<- renderPlot({
    ...
    plot12 <- ggplot(dfic, aes(x = ICU, y=n, fill = DEAD)) +
      geom_bar(position = "stack", stat ="identity") +  
      labs(title = "Stacked Bar Chart of ICU admission vs Death Status",
           x = "icu",
           y = "Count",
           fill = "Death Status") +
      theme_minimal()
    ...
    plot13 <- ggplot(dfu, aes(x = USMER, y=n, fill = DEAD)) +
      geom_bar(position = "stack", stat ="identity") +  
      labs(title = "Stacked Bar Chart of Medical Treatment Degree vs Death Status",
           x = "degree",
           y = "Count",
           fill = "Death Status") +
      theme_minimal()
      ...
      geom_bar(position = "stack", stat ="identity") +  
      labs(title = "Stacked Bar Chart being connected to a ventilator vs Death Status",
           x = " ",
           y = "Count",
           fill = "Death Status") +
      theme_minimal()
